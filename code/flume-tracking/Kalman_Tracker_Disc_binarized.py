"""
Core tracking script: Kalman_Tracker_Disc_binarized.py

Purpose
-------
Tracks disc trajectories from binarised video frames using contour extraction, quadrilateral fitting, orientation estimation and Kalman smoothing.

Inputs
------
Disc videos (.mp4), calibration JSON, manually selected/defined seed frame and particle centre.
...
--------------------
The imaged disc motion is treated as approximately planar; apparent projected area and orientation are derived from the detected silhouette; the Kalman filter assumes locally smooth constant-velocity motion between frames.
...
ENABLE_IMSHOW = True
DEFAULT_VIDEO_PATH = 'C:\\Users\\User\\Downloads\\4.5_inch_per_second_flow\\disc drop 1.mp4'
DEFAULT_CALIBRATION_JSON = 'C:\\Users\\User\\Downloads\\rod_calibration_snap_fit\\calibration.json'
DEFAULT_OUT_DIR = 'C:\\IP Work\\4.5_Inchps_Discs_Binarised\\disc_drop_1_binarized_tracker_output'
DEFAULT_REAL_FPS = 240.0
DEFAULT_FRAME_STRIDE = 1
DEFAULT_START_FRAME = 0
DEFAULT_OBJECT_MODE = 'disc'
...
def evenly_spaced_frame_indices(start_frame, end_frame, count):
    ...
    count = int(count)
    if count <= 0 or end_frame < start_frame:
        return []
    if count == 1:
        return [start_frame]
...
def preprocess_bright_binary_roi(gray_roi, params, background_gray_roi=None):
    ...
    return {'background_roi': background_roi_u8, 'deviation_map': deviation_map, 'blur': blur, 'bg': bg, 'bright_map': bright_map, 'threshold_used': float(threshold_used), 'gray_threshold_used': float(gray_threshold_used), 'bw_bright': bw_bright, 'bw_gray': bw_gray, 'bw_raw': bw_raw, 'bw_clean': bw, 'bw': bw_filled}
...
def detect_in_raw_roi(frame_bgr, pred_x, pred_y, H, px_per_mm, object_mode, sphere_color_mode, disc_diameter_mm, sphere_diameter_mm, stage_params, rod_anchor_uv=None, disc_background_gray=None):
    ...
    disc_background_roi = None
    if disc_background_gray is not None:
        disc_background_roi = disc_background_gray[y0:y1, x0:x1]
    aux_a = aux_b = aux_c = None
    binary_dbg = None
    red_dbg = None
    if object_mode == 'disc':
        binary_dbg = preprocess_bright_binary_roi(gray_roi, stage_params, background_gray_roi=disc_background_roi)
        bw = binary_dbg['bw']
...
def search_start_detection(cap, start_frame, click_xy, H, px_per_mm, object_mode, sphere_color_mode, disc_diameter_mm, sphere_diameter_mm, stage_params, search_radius, disc_background_gray=None):
    ...
    for offset in offsets:
        ...
        if not ok:
            continue
        dets, debug = detect_in_raw_roi(frame, click_xy[0], click_xy[1], H, px_per_mm, object_mode, sphere_color_mode, disc_diameter_mm, sphere_diameter_mm, stage_params, rod_anchor_uv=None, disc_background_gray=disc_background_gray)
...
def score_detection(det, pred_x, pred_y, object_mode, track_angle_deg=None, track_length_mm=None, expected_disc_diameter_mm=None, expected_sphere_diameter_mm=None):
    ...
    if object_mode == 'sphere':
        ...
        return dist + 6.0 * d_diam + circularity_penalty
    return dist
...
def save_outputs(dfp, out_dir, object_mode):
    ...
    if object_mode in {'rod', 'disc'}:
        angle_label = 'rod angle (deg)' if object_mode == 'rod' else 'disc orientation (deg)'
        angle_title = 'rod angle vs time' if object_mode == 'rod' else 'disc orientation vs time'
        omega_title = 'rod angular velocity vs time' if object_mode == 'rod' else 'disc angular velocity vs time'
        depth_title = 'rod angle vs depth' if object_mode == 'rod' else 'disc orientation vs depth'
        save_plot(dfp['t_rel_s'], dfp['angle_smooth_deg'], 'time from first detection (s)', angle_label, angle_title, reg('angle_vs_time.png'))
        save_plot(dfp['t_rel_s'], dfp['omega_deg_s'], 'time from first detection (s)', 'angular velocity (deg/s)', omega_title, reg('omega_vs_time.png'))
        plt.figure()
...
def main():
    ...
    disc_background_gray = None
    disc_background_info = None
    if cfg['object_mode'] == 'disc' and bool(cfg.get('enable_disc_background_subtraction', DEFAULT_ENABLE_DISC_BACKGROUND_SUBTRACTION)):
        background_seconds = float(cfg.get('disc_background_seconds', DEFAULT_DISC_BACKGROUND_SECONDS))
        ...
        if background_window_frames > 0:
            disc_background_gray, disc_background_info = build_average_background_gray(cap, cfg['start_frame'], background_window_frames, int(cfg.get('disc_background_samples', DEFAULT_DISC_BACKGROUND_SAMPLES)), margin_frames=int(cfg.get('disc_background_margin_frames', DEFAULT_DISC_BACKGROUND_MARGIN_FRAMES)), keep_fraction=float(cfg.get('disc_background_keep_fraction', DEFAULT_DISC_BACKGROUND_KEEP_FRACTION)))
        else:
            disc_background_info = {'status': 'disabled', 'reason': 'disc_background_seconds<=0'}
        if disc_background_gray is not None:
            cv2.imwrite(os.path.join(cfg['out_dir'], 'debug_background_average.png'), np.clip(disc_background_gray, 0, 255).astype(np.uint8))
        if disc_background_info is not None:
            with open(os.path.join(cfg['out_dir'], 'debug_background_info.json'), 'w', encoding='utf-8') as f:
                json.dump(disc_background_info, f, indent=2)
    ...
    if cfg['use_preset_click']:
        ...
        if cfg['object_mode'] == 'disc':
            start_gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
            start_binary = preprocess_bright_binary_roi(start_gray, acquire_params, background_gray_roi=disc_background_gray)['bw']
            init_click = pick_initial_click_binary(start_binary, mode_label)
        else:
            init_click = pick_initial_click_raw(frame, mode_label)
    dets, debug = detect_in_raw_roi(frame, init_click[0], init_click[1], H, px_per_mm, cfg['object_mode'], cfg['sphere_color_mode'], cfg['disc_diameter_mm'], cfg['sphere_diameter_mm'], acquire_params, rod_anchor_uv=None, disc_background_gray=disc_background_gray)
    if len(dets) == 0 and int(cfg.get('start_search_frames', 0)) > 0:
        found_frame_idx, found_frame, dets, debug = search_start_detection(cap, cfg['start_frame'], init_click, H, px_per_mm, cfg['object_mode'], cfg['sphere_color_mode'], cfg['disc_diameter_mm'], cfg['sphere_diameter_mm'], acquire_params, cfg['start_search_frames'], disc_background_gray=disc_background_gray)
        if len(dets) > 0:
    ...
    if len(dets) == 0:
        ...
        if 'roi_hsv' in debug:
            ...
            cv2.imwrite(os.path.join(cfg['out_dir'], 'debug_start_bw.png'), debug['bw'])
        raise RuntimeError('No valid contour found near the selected start point on the binarised frame. Adjust start frame or disc binary settings.')
    ...
    binary_vw = cv2.VideoWriter(out_binary_mp4, fourcc, float(file_fps / cfg['frame_stride']), (w_raw, h_raw), True) if cfg['object_mode'] == 'disc' else None
    rows = []
    ...
    while True:
        ...
        dets, debug = detect_in_raw_roi(frame, pred_x, pred_y, H, px_per_mm, cfg['object_mode'], cfg['sphere_color_mode'], cfg['disc_diameter_mm'], cfg['sphere_diameter_mm'], search_params, rod_anchor_uv=track.rod_anchor_uv if cfg['object_mode'] == 'rod' else None, disc_background_gray=disc_background_gray)
        if cfg['object_mode'] == 'disc' and track.misses >= int(search_params.get('reacquire_fullframe_after_misses', 99)):
            reacquire_params = stage_params.copy()
            reacquire_params['roi_half_size_px'] = max(h_raw, w_raw)
            reacquire_dets, reacquire_debug = detect_in_raw_roi(frame, pred_x, pred_y, H, px_per_mm, cfg['object_mode'], cfg['sphere_color_mode'], cfg['disc_diameter_mm'], cfg['sphere_diameter_mm'], reacquire_params, rod_anchor_uv=None, disc_background_gray=disc_background_gray)
            if reacquire_dets:
                dets = reacquire_dets
        ...
        if cfg['object_mode'] == 'disc':
            ...
            full_binary = preprocess_bright_binary_roi(full_gray, stage_params, background_gray_roi=disc_background_gray)
        ...
        if cfg['object_mode'] == 'disc' and track.misses > 0:
        ...
        if cfg['object_mode'] == 'rod':
            ...
            row.update({'diameter_mm': track.diameter_mm, 'major_axis_mm': track.length_mm, 'minor_axis_mm': track.width_mm, 'angle_deg': track.angle_deg})
        else:
